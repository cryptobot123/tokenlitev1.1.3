...
<div class="page-ath-form">
    ...
    <form class="register-form validate validate-modern" method="POST" action="{{ route('social.register') }}" id="register">
        ...
        <div class="input-item">
            <input type="text" class="input-bordered{{ $errors->has('name') ? ' input-error' : '' }}" name="name" value="{{ $user->getName() }}" required>
        </div>
        <div class="input-item">
            <input type="email" class="input-bordered{{ $errors->has('email') ? ' input-error' : '' }}" name="email" value="{{ $user->getEmail() }}" required>
        </div>

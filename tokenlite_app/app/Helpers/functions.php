...
/* @function is_method_valid()  @version v1.2  @since 1.0 */
...
/* @function _format()  @version v1.3  @since 1.0 */
...
/* @function to_num()  @version v1.2  @since 1.0.3 */
if (!function_exists('to_num')) {
    function to_num($num, $decimal='max', $thousand='', $trim = true, $point='.')
    {
        return _format(['number'=> $num, 'decimal' => $decimal, 'thousand' => $thousand, 'trim' => $trim, 'point' => $point, 'end' => true]);
    }
}

/* @function to_num_round()  @version v1.1  @since 1.1.2 */
if (!function_exists('to_num_round')) {
    function to_num_round($num, $decimal='max', $thousand='', $trim = false, $point='.')
    {
        return _format(['number'=> $num, 'decimal' => $decimal, 'thousand' => $thousand, 'trim' => $trim, 'point' => $point, 'end' => true]);
    }
}
...
/* @function is_payment_method_exist()  @version v1.3  @since 1.0 */
if (!function_exists('is_payment_method_exist')) {
    function is_payment_method_exist($method = '')
    {
        ...
        $is_active_mm = ($manual->status == 'active' && ($is_pm_eth||$is_pm_btc||$is_pm_ltc||$is_pm_xrp||$is_pm_xlm||$is_pm_bch||$is_pm_bnb||$is_pm_trx||$is_pm_usdt||$is_pm_usdc||$is_pm_dash)) ? true : false;

        $is_payment_method_exist = PaymentMethod::where('status', 'active')->count() ? true : false;

        // Return Manual
        if ($method == 'manual' || $method == 'manual_eth' || $method == 'manual_btc' || $method == 'manual_ltc' || $method == 'manual_xrp' || $method == 'manual_xlm' || $method == 'manual_bch'  || $method == 'manual_bnb' || $method == 'manual_trx' || $method == 'manual_usdt' || $method == 'manual_usdc' || $method == 'manual_dash' ) {
...
/* @function short_to_full()  @version v1.3  @since 1.0 */
...
/* @function wallet_to_currency()  @version v1.2  @since 1.1.2 */
...
/* @function get_refer_id  @version v1.0  @since 1.1.0 */
...
/* @function get_lang()  @version v1.0  @since 1.1.3 */
...
if (!function_exists('api_route')) {
    function api_route($name='') {
        $url = route('api.'.$name, ['secret' => get_setting('site_api_key')]); 
        return $url;
    }
}

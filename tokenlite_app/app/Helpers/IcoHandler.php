...
class IcoHandler
{
    ...
    const ICU_PATH = 'https://api.softnio.com/check/envato/5hcPWdxQ';
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        if(file_exists(storage_path('installed'))){
            ...
            if($request->is('admin*') && !$request->is('admin/tokenlite-register') && !$request->is('admin/system-info')) {
                ...
                if( $skip != true ){
                    if( !$this->check_body() || $this->app_demo_unlock() ){
                        return redirect()->route('admin.tokenlite');
                    }
                }
            }
            return $next($request);
    ...
    public static function _message()
    {
        if (empty(env_file()) || empty(app_key(1)) || empty(app_key(2)) || !nio_feature('cool') ) {
            $text = "<!-- TokenLite v" . str_replace('.', '', config('app.version')).config('app.installed').config('app.update') . ". Application Developed by Softnio -->\n";
        } else {
    ...
    public static function panel_info($output = '')
    {
        ...
        $valid = gws('tokenlite_credible', 'none');
        $return = $name;
        ...
        try {
            if(serverOpenOrNot()){
                ...
                $send = $client->get(get_transport(), ['query' => [
                    'domain' => $domain, 'purchase_code' => get_setting('env_pcode'), 'activation_code' => get_setting('nio_lkey'),
                    'app_code' => get_setting('env_ptype'), 'app_name' => site_info('name'), 'app_url' => url('/'), 'app_version' => config('app.version')
                ]]);
                $response = $send->getBody();
                $result = json_decode($response);
                if($result->status == 'active' && $this->cris_cros($this->getDomain(), $result->valid)){
                    add_setting('tokenlite_update', $result->timestamp); add_setting('nio_lkey', $result->code); 
                    add_setting('env_ptype', (substr($result->code, 3, 5))); add_setting('tokenlite_credible', $result->valid);
                    // info($result->message);
                    return true;
                }else{
                    $time = get_setting('tokenlite_update', time() + 3600);
                    $text = strlen(gws('env_ptype')) > 1 ? substr(gws('env_ptype'), 0, -1) : gws('env_ptype');
                    add_setting('tokenlite_update', $time); add_setting('env_ptype', $text);
                    if(strlen($text) == 1){ add_setting('nio_lkey', $this->new_random()); }
                    // info($result->message);
                    return false;
                }
            }
        ...
        try {
            if(serverOpenOrNot()){
                ...
                if($result->status == true && $this->cris_cros($this->getDomain(), $result->valid)){
                    add_setting('tokenlite_update', $result->timestamp);
                    add_setting('env_pcode', $request->purchase_code);
                    add_setting('env_ptype', (substr($result->code, 3, 5)));
                    add_setting('nio_lkey', $result->code);
                    add_setting('tokenlite_credible', $result->valid);
                    add_setting('env_uname', $request->name);
                    add_setting('nio_email', $request->email);
                    $text = $result->message;
                    if($request->ajax()){
    ...
    /* @function get_manual_payment()  @version v1.2 */
    public static function get_manual_payment($type, $ext = '', $active = true)
    {
        ...
        if ($type == 'usd' || $type == 'eur' || $type == 'gbp' || $type == 'cad' || $type == 'aud' || $type == 'try' || $type == 'rub') {
    ...
    protected function find_the_path($domain)
    {

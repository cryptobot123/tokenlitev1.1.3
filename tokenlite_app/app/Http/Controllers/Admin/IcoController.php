    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($validator->errors()->has('name')) {
                ...
                $msg = NioTrans::do('start date', $validator->errors()->first(),['format' => 'm/d/Y']);
            } elseif ($validator->errors()->has('end_date')) {
                $msg = NioTrans::do('end date', $validator->errors()->first(),['format' => 'm/d/Y', 'date' => $request->start_date]);
            } elseif ($validator->errors()->has('total_tokens')) {
                $msg = NioTrans::do('total tokens', $validator->errors()->first(),['value' => 0]);
            } elseif ($validator->errors()->has('base_price')) {
                $msg = NioTrans::do('base price', $validator->errors()->first(),['value' => 6]);
            } elseif ($validator->errors()->has('display_mode')) {
                $msg = NioTrans::do('display mode', $validator->errors()->first());
            } elseif ($validator->errors()->has('min_purchase')) {
                $msg = NioTrans::do('min purchase', $validator->errors()->first(),['min' => 5]);
            } elseif ($validator->errors()->has('max_purchase')) {
                $msg = NioTrans::do('max purchase', $validator->errors()->first(),['max' => 10]);
            } elseif ($validator->errors()->has('soft_cap')) {
                $msg = NioTrans::do('soft cap', $validator->errors()->first(),['min' => 10]);
            } elseif ($validator->errors()->has('hard_cap')) {
                $msg = NioTrans::do('hard cap', $validator->errors()->first(),['min' => 50, 'max' => $request->total_tokens]);
            } else {
        ...
        if ($type == 'token_details') {
            ...
            if ($validator->fails()) {
                ...
                if ($validator->errors()->has('token_name')) {
                    $msg = NioTrans::do('token name', $validator->errors()->first(), ['min' => 4]);
                } elseif ($validator->errors()->has('token_symbol')) {
                    $msg = NioTrans::do('token symbol', $validator->errors()->first(), ['min' => 2]);
                } else {
